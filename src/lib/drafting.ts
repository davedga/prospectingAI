...
- Never say a brand is "already" on TikTok Shop or "live on" TikTok Shop — both read weak/vague. State the fact plainly instead: name the real GMV figure if known ("is doing [X] monthly GMV on TikTok Shop"), or just "is on TikTok Shop" if no figure is known.
...
- Question bank, when a question is used — varied so it doesn't read identically every time. It branches on TWO things: whether the brand is already on TikTok Shop (getting this right matters — a brand already live on TikTok Shop should never get a "have you considered getting on TikTok Shop" style question, it reads like you didn't do the research), and the contact's seniority, which only changes how the question ENDS. Rotate the ending across these, matched to the contact's role — don't reuse the same ending every time:
  ...
  - "...a priority for the team?" (junior/operational)
  ...
  - TTS-status branch for the rest of the question:
    ...
    - Already on TikTok Shop, GMV known: "Is scaling to [the real known monthly GMV figure, or 'six and seven-figure' if that's the framing being used] monthly GMV on TikTok Shop [ending]"
    - Already on TikTok Shop, GMV not known: "Is scaling further on TikTok Shop [ending]"
  - Later touches, not on TTS: "Is scaling on TikTok Shop a priority for the team going into the holiday season?"
  - Later touches, already on TTS: "Is scaling further on TikTok Shop something the team's been considering?" or "Is this currently a priority for [you/the team/Company]?"
...
- Use "talk with," not "speak with," in any redirect-to-someone-else ask.
...
2. Category + observation, one sentence: "Ahead of Q4, I've been looking closely at the [category] category and noticed [Company] [observation]." ${getSeasonalFramingInstruction()} The observation MUST match their actual TikTok Shop status (given below) — three real cases, never blur them:
   ...
   - Already on TikTok Shop, real GMV figure genuinely known/sourced: state it directly — "...noticed [Company] is doing [X] monthly GMV on TikTok Shop." Never invent or round-estimate this figure.
