...
export async function draftFirstEmail(
  ...
  const firstTouchStructure = `First-touch structure, roughly 35-45 words total, hard cap 50. This is a deliberate, large cut from any longer target you might infer from the pattern below — every clause has to earn its place, cut ruthlessly rather than trim politely. Write it as ONE flowing set of sentences, not a checklist of disconnected beats stitched together with connector phrases — a reader should never be able to point to a sentence and say "that's the Q4 clause" or "that's the bolt-on line." Fit these beats into that budget, compressed into as few clauses as the words allow:

1. Who we are — one short clause: "I'm [name] from DGA, we work with ByteDance to help brands scale on TikTok Shop." Paraphrase naturally, don't copy verbatim every time, but keep it to a single clause — no elaboration on what DGA does beyond this.
2. Skip the optional track-record (six/seven-figure) line by default at this length — it costs 15-20 words on its own, more than a third of the entire budget. Only include it on an email that has nothing else going for it (thin record, generic category-level observation) where the credibility is worth the space; when the brand-specific observation is strong, always skip it.
3. Why now + the observation, fused into one clause, not two — ${getSeasonalFramingInstruction()} Don't spend words explaining that you "did research" or "looked into the category" — just state the observation directly with the timing folded in ("Q4's coming up and [Company]'s [X] looks like..." not "I've been doing research and noticed..."). This must match their actual TikTok Shop status (given below) — not on TikTok Shop at all, on but not prioritized, on but underdeveloped for their size, or another real condition; never default to "not on TikTok Shop yet" if the record shows otherwise. If the record has a real, specific reason this brand fits (a bundle-friendly line, a clear hero SKU, a notable listing structure) name that thing directly instead of a generic category observation, without citing numbers. If the record is thin, keep it at the category level rather than inventing brand-specific detail.
4. The ask — exactly one soft question from the question bank in the rules above, and the only question mark in the email. Keep it to the short form ("Has TikTok Shop been on your radar?") — no extra clause tacked onto the question itself.`;
