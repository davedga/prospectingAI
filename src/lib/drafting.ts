...
export async function draftFirstEmail(
  ...
  const firstTouchStructure = `First-touch structure, roughly 55-100 words total (the higher end when the optional track-record line is included). Write it as ONE flowing set of sentences, not a checklist of disconnected beats stitched together with connector phrases — a reader should never be able to point to a sentence and say "that's the Q4 clause" or "that's the bolt-on line." Build it in this order:

1. Who we are — "I'm [name] from DGA. My team and I work on behalf of ByteDance (TikTok's parent company) to help identify and scale brands with strong potential to perform well on TikTok Shop." Use this framing (paraphrased naturally, not copied verbatim every time) rather than the older generic "we work with/research brands on TikTok Shop" framing.
2. Optional track-record line — the six/seven-figure monthly revenue range statement from the rules above, when it earns its place in this particular email (see the rule above for when to include vs skip it).
3. Why now, into the observation, in one motion — "With Q4 coming up, I've been doing some research in your category and [observation]..." ${getSeasonalFramingInstruction()} This should read as one continuous thought, not a separate clause tacked onto the end of a sentence. Never use a generic hedge like "even as a few brands in the category are gearing up ahead of Q4" — be direct about the timing instead of softening it into a throwaway aside. Vary the exact transition verb across emails instead of reusing the same one every time — "was surprised to see," "noticed that," "noticed [Company]..." are all fine, just don't default to the same phrase mechanically.
4. The observation itself — combine the category/brand-fit reasoning and their CURRENT TikTok Shop status into ONE flowing observation, not two separate points or two separate questions. This must match their actual status (given below as "TikTok Shop status") — not on TikTok Shop at all, on TikTok Shop but doesn't look prioritized, on TikTok Shop but underdeveloped for their size, or another real condition. Never default to "not on TikTok Shop yet" if the record shows otherwise. If the record below includes a real, specific reason this brand looks like a good fit for TikTok Shop beyond just the category (a bundle-friendly product line, a clear hero SKU, a notable listing structure, or economics that seem favorable) — surface that specific reason here instead of a generic category-level observation, without citing brand-specific numbers. If the record is thin (no strong company-specific signal beyond category and TikTok Shop status), keep the observation at the category level rather than inventing brand-specific detail. When the fit is close to obvious given the record (a hero SKU and price point that map cleanly onto what already works on TikTok Shop, for example), it's fine for the observation to say so plainly rather than hedging it into vague language.
5. The ask — exactly one soft question from the question bank in the rules above, and the only question mark in the email. The goal is an easy yes/no reply that opens a conversation, not a detailed strategic answer.`;
