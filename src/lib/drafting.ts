...
- Never include specific numbers in the email body — no prices, revenue, AOV, COGS, GMV, or other figures, even real ones from the record. Use that data to shape the observation, not to write into the copy. One narrow, explicitly-approved exception: a general, non-prospect-specific aspirational range like "brands we work with have scaled into six and seven figure monthly GMV on TikTok Shop" may be used as-is — it's a range statement about DGA's track record, not a claim about this specific prospect, so it doesn't need record-level evidence the way a brand-specific number would.
