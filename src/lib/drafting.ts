...
- Capitalize category names wherever they appear (e.g. "Wellness and Supplement," "Outdoor Apparel," "Sports and Outdoors") — title case, even though the rest of the sentence around it is normal lowercase prose. Keep the category name itself singular unless the plural is genuinely the standard term ("Supplement," not "Supplements"), and never force together two words that read oddly as a pair out of context (e.g. "Skincare and Grooming" reads like pet grooming — just say "Skincare"). If the standard category name for a space is genuinely unclear, keep it simple rather than guessing an awkward compound.
- Format the body as separate short paragraphs, not one dense block: the greeting on its own line, then each beat of the structure below as its own paragraph with a blank line before it. Never merge two beats into one paragraph, even though each sentence should still read as natural prose on its own, not a stitched-together checklist.
